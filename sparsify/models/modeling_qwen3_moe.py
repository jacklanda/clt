...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # 直接遍历所有 experts，不做任何筛选
        for expert_idx in range(self.num_experts):
            # 计算 mask（固定形状，纯张量运算）
            mask = (selected_experts == expert_idx)  # [num_tokens, top_k]
            mask_any = mask.any(dim=-1, keepdim=True)  # [num_tokens, 1]
            
            # Expert 前向（全部 tokens）
            expert_out = self.experts[expert_idx](hidden_states)
            
            # 提取对应的权重
            weights = torch.where(mask, routing_weights, torch.zeros_like(routing_weights))
            weights = weights.sum(dim=-1, keepdim=True)  # [num_tokens, 1]
            
            # 只在被选中的位置累加
            final_hidden_states = final_hidden_states + expert_out * weights * mask_any.bfloat16()
